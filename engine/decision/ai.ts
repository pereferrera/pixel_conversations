...
export function decisionPrompt(context: DecisionContext): string {
  ...
  return [
    ...
    "CAMERA TRUTH: context.scene.visibleWorld is the complete physical world visible in this simulation. Treat its setting, visibleFeatures, and boundaries as literal constraints, not creative suggestions. Do not invent nearby rooms, floors, stairs, doors, businesses, vehicles, objects, facilities, weather changes, or environmental features that are absent from visibleWorld.",
    "The only physical actions that can occur are the returned changes: characters may talk, move between declared scene positions, sit, stand, or sleep. Visible background features may be noticed, looked at, pointed toward, or discussed, but they cannot be entered, picked up, operated, changed, or used unless an allowed action explicitly represents that interaction.",
    "Dialogue may mention memories, plans, hypothetical actions, and off-screen places, but those references do not make them present events. A character may say they intend to go downstairs later; the summary and event must not claim that they went downstairs or left the scene.",
    "Every decision summary and addEvent summary must report only local, present-tense occurrences that are visible in the fixed scene and directly supported by the returned changes. Never imply unrepresented travel, object interaction, arrival, departure, scene transition, or off-screen action.",
    ...
    "Use addEvent only when a brief caption is needed to clarify a local occurrence already fully represented by the other returned changes. It cannot introduce another person, animal, object, sound source, physical action, environmental change, or off-screen occurrence. At most one event may be added per step; it remains visible for the resulting frame and expires before the next simulation step.",
    "The rendered scene and background never change. Never describe a shift in environment, weather, lighting, architecture, furniture, or backdrop. Do not use prose to simulate capabilities the state does not have.",
